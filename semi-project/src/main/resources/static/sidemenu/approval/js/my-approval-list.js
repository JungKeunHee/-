async function fetchApprovalData() {
    try {
        const response = await fetch('/api/my-approval-list');
            ...
            approvals.forEach(item => {
                ...
                listItem.addEventListener('click', () => {
                    ...
                    if (item.type.includes("연차") || item.type.includes("휴가")) {
                        // 해당 URL로 이동
                        window.location.href = `/myVacRequest?no=${item.documentNo}`;
                    } else if (item.type.includes("결혼") || item.type.includes("조사")) {
                        // "경사" 또는 "조사"일 경우 해당 URL로 이동
                        window.location.href = `/myCacRequest?no=${item.documentNo}`;
                    } else if (item.type.includes("연장근무") || item.type.includes("연장")) {
                        // "연장근무" 또는 "연장"일 경우 해당 URL로 이동
                        window.location.href = `/myOverTimeRequest?no=${item.documentNo}`;
                    } else if (item.type.includes("퇴직")) {
                        // "퇴직"일 경우 해당 URL로 이동
                        window.location.href = `/myRetireMentRequest?no=${item.documentNo}`;
                    }else {
